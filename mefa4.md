...
`mefa4` is a reimplementation of the S3 object classes found in the \pkg{mefa} \proglang{R} package.
...
The aim of the `mefa` and `mefa4` packages are to help in storing cross tabulated
...
separately manipulating 2--3 pieces of R objects. By doing so, the chances of errors are reduced.
...
the `mefa` package and take advantages of sparse matrices from the `Matrix` package.
...
## Comparison of S3 and S4 classes
...
attributes(s)
```
These attributes ensure that the cross-tab made by the function `mefa()` creates a proper
...
The `stcs` step is almost redundant, and inefficient relative to the \code{stats::xtabs}
